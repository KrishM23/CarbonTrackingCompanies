...
def export_pdf(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    story = []
    story.append(Paragraph("Vapor", title))
    story.append(
        Paragraph(
            f"Company emissions progress report · {company.name}"
    ...
    story.append(
        Paragraph(
            ...
            "capital-turnover S-curves. Figures are company-reported; Vapor "
    ...
    story.append(
        Paragraph(
            f"Generated by Vapor for {company.name} · Confidential · Internal ESG use",
            subtitle,
        )
    )
    ...
    doc.build(story)
    buf.seek(0)
    safe = "".join(c if c.isalnum() or c in "-_" else "-" for c in company.name.lower().strip())
    filename = f"vapor-{safe}-emissions-report.pdf"
